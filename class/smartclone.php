...
class SmartClone {
	...
	function execute() {
		...
		if (chmod($module_dir, 0777)) {
			...
			include_once(SMARTCLONE_ROOT_PATH . 'class/ftpchmod.php');
			$ftpChmod = new SmartcloneFtpchmod();
			if (!$ftpChmod->changePermissions(0777)) {
				$this->setError(_AM_SCLONE_CHANGE_PERMISSION_FAILED . "(ftp)");
				return false;
		...
		if (!chmod($module_dir, $fileperm)) {
			// try to change to chmod the folder by FTP
			include_once(SMARTCLONE_ROOT_PATH . 'class/ftpchmod.php');
			$ftpChmod = new SmartcloneFtpchmod();
			$ftpChmod->changePermissions($fileperm);
